...
def main():
    ...
    st.write("To use this tool, you'll need a free AssemblyAI account. Sign up using your institutional/work email and copy your API key into the field provided. We'll handle the rest!")
    ...
    with st.form("APP"):
        ...
        sub_limit = st.slider("Character limit per line:", min_value=20, max_value=150, value=75)
        ...
        if submitted and api_key and audio_file:
            subtitles = transcribe_and_generate_subtitles(api_key, sub_limit, audio_file)
            if subtitles:
                st.success("Subtitles generated successfully!")
                output_file_name = os.path.splitext(audio_file.name)[0] + '_SUBTITLES.srt'
